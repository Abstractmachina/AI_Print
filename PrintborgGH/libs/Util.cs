...
namespace PrintborgGH.libs {
	internal class Util {
		internal static class SpatialSlur {
			/// <summary>
			/// 
			/// </summary>
			/// <param name="m"></param>
			...
			internal static double[] ReparamField(double[] vals) {
				...
				for (int i = 0; i < vals.Length; i++) {
					if (vals[i] > 0) newvals[i] = (global::SpatialSlur.SlurCore.SlurMath.Remap(vals[i], 0, max, 0, 1));
					else newvals[i] = (global::SpatialSlur.SlurCore.SlurMath.Remap(vals[i], min, 0, -1, 0));
				}
